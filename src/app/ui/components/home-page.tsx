...
function StructuredData({
  ...
  const inLanguage = language === 'ja' ? 'ja-JP' : 'en-US'
  const data = {
    '@context': 'https://schema.org',
    '@type': 'Organization',
    name: 'Ark Studios',
    legalName: 'Ark Studios LLC',
    url: 'https://arkstudios.co.jp/',
    logo: 'https://arkstudios.co.jp/images/favicons/favicon-256x256.png',
    image: 'https://arkstudios.co.jp/opengraph-image.jpg',
    description,
    inLanguage,
    sameAs: ['https://twitter.com/ArkStudiosLLC', 'https://github.com/ArkStudiosLLC'],
  }
...
export default async function HomePage({ language }: { language: Language }) {
  const description = (await getDictionary(language)).Metadata.Template.description

  return (
    <div className='flex flex-col select-none'>
      <StructuredData description={description} language={language} />
      <NavigationBar language={language} pathname='' />
      <main
        id='main-content'
